...
const columns = [
    ...
    { id: 'students', label: "רשומים", minWidth: 120, maxWidth: 130, align: 'center' },
    { id: 'date', label: "תארכי מפגשים", minWidth: 170, maxWidth: 170, align: 'center' },
...
export default function ActivityFeedbackForAdmin(props) {
    ...
    const rows = activitiesFeedbacks.map((activity, index) => {
        return (createDataAdmin(activity.owner, activity.phone_number, activity.title, activity.email,
            <p>תאריך - {activity.date.substring(0, 10).split("-").reverse().join("-")} שעה - {activity.date.substring(11)}</p>,
            <Scrollbars style={{ width: 130, height: 200, float: "right" }}>{activity.form.map((student) => <div><p>{student[0]}</p></div>)}</Scrollbars>,
            <div>
    ...
    const fetchActivitiesFeedbacks = async () => {
        try {
            const activitiesFeedbacksData = await API.graphql(graphqlOperation(listActivityFeedbacks));
            const activitiesFeedbacksList = activitiesFeedbacksData.data.listActivityFeedbacks.items;
    ...
    function createDataAdmin(name, phoneNumber, activityName, email, date, students, buttons) {
